...
export function FilterBar({
  ...
  return (
    <div className="flex flex-col gap-2 border-b border-line bg-paper-raised px-4 py-3">
      <div className="flex gap-2">
        <input
          ...
          className="w-full rounded-lg border border-line-strong bg-paper px-3 py-2 text-sm text-ink outline-none placeholder:text-ink-faint focus:border-pitch focus:ring-2 focus:ring-pitch/15"
        />
        <button
          type="button"
          onClick={onNearMe}
          disabled={nearMeState === "locating"}
          className={`shrink-0 rounded-lg px-3 py-2 text-sm font-semibold transition disabled:opacity-60 ${
            nearMeState === "active"
              ? "bg-pitch text-paper"
              : "border border-line-strong text-ink-soft hover:border-pitch hover:text-pitch-deep"
          }`}
        >
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-xs">
        <select
          ...
          className="rounded-full border border-line-strong bg-paper px-2.5 py-1 font-semibold text-ink-soft outline-none focus:border-pitch"
        >
